...
public class MyApplyForPayActivity extends BaseAppCompatActivity {
        ...
        Subscription subscription = NetWork.getPayService().dhOrderPay(mPayFlag, realm.where(User.class).findFirst().getUserId()).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Observer<Map<String, String>>() {
            ...
            public void onNext(Map<String, String> map) {
                ...
                if (!StringUtils.isEmpty(map.get("orderInfo"))) {
                    new Pay(MyApplyForPayActivity.this, map.get("orderInfo")).payV2();
                }
            }
        });
    ...
    private void setBg(int flag) {
        ...
        switch (flag) {
            case R.id.payMoney_left_lin:
                mPayFlag = "01";
                payMoneyLeftLin.setBackground(MyApplyForPayActivity.this.getResources().getDrawable(R.drawable.radio_red_border_shap));
                totalPayTv.setText("120");
                break;
            case R.id.payMoney_med_lin:
                mPayFlag = "02";
                payMoneyMedLin.setBackground(MyApplyForPayActivity.this.getResources().getDrawable(R.drawable.radio_red_border_shap));
                totalPayTv.setText("80");
                break;
            case R.id.payMoney_right_lin:
                mPayFlag = "03";
                payMoneyRightLin.setBackground(MyApplyForPayActivity.this.getResources().getDrawable(R.drawable.radio_red_border_shap));
